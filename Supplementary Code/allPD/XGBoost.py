...
# XGBoost expects a tuple or list of length equal to number of features
monotone_constraints_tuple = tuple(monotone_constraints)
...
# Train final XGBoost model WITH monotonic constraints
xgb = XGBRegressor(
    **study.best_params,
    monotone_constraints=monotone_constraints_tuple,   # apply physical constraints
    random_state=100,
    n_jobs=-1,
    verbosity=0
)
...
results = pd.DataFrame([['XGBoost (with monotonic constraints)',
...
for fold, (train_index, val_index) in enumerate(kf.split(X_train_scaled), 1):
    ...
    y_fold_train_pred = xgb_fold.predict(X_fold_train)
    y_fold_val_pred = xgb_fold.predict(X_fold_val)
    ...
    plt.title(f'XGBoost – Cross-validation Fold {fold}', fontsize=13, fontweight='bold')
