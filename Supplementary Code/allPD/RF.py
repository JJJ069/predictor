...
# Standardisation (consistent with other models)
...
# Hyperparameter optimization with Optuna (TPE) for Random Forest
def objective(trial):
...
rf = RandomForestRegressor(**study.best_params, random_state=100, n_jobs=-1)
...
for fold, (train_index, val_index) in enumerate(kf.split(X_train_scaled), 1):
    ...
    rf_fold = RandomForestRegressor(**study.best_params, random_state=100, n_jobs=-1)
    rf_fold.fit(X_fold_train, y_fold_train)

    y_fold_train_pred = rf_fold.predict(X_fold_train)
    y_fold_val_pred = rf_fold.predict(X_fold_val)
...
# SHAP interpretability analysis (for Random Forest)
import shap
import matplotlib.pyplot as plt
import warnings
import numpy as np
