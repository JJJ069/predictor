...
#    NOTE: SVR does NOT support monotonic constraints.
#    They are defined here for reference but cannot be applied.
...
print("Monotone constraints (order matches features) – NOT APPLIED to SVR:", monotone_constraints)

# GridSearchCV for SVR (with scaling inside pipeline)
param_grid = {
    'svr__kernel': ['rbf', 'linear', 'poly'],
    'svr__C': [0.1, 1, 10, 100],
    'svr__epsilon': [0.01, 0.1, 0.5],
    'svr__gamma': ['scale', 'auto'],  # effective for rbf/poly
    'svr__degree': [2, 3]             # effective for poly
}

base_pipeline = make_pipeline(
    StandardScaler(),
    SVR()   # SVR does not accept random_state
)
...
# Get the best estimator (pipeline with scaler + SVR)
best_svr = grid_search.best_estimator_

# Predictions and clip negative values to zero
y_train_pred_raw = best_svr.predict(X_train)
y_test_pred_raw = best_svr.predict(X_test)

# Clip negative predictions to 0 (non‑negative constraint)
y_train_pred = np.maximum(y_train_pred_raw, 0)
y_test_pred = np.maximum(y_test_pred_raw, 0)

# Evaluation metrics (computed on clipped predictions)
...
results = pd.DataFrame([['SVR (clipped non‑negative predictions)',
                       ...
                       columns=['Method', 'Training MSE', 'Training RMSE',
                                ...
                                'Test MSE', 'Test RMSE',
                                'Test MAE', 'Test R2'])
print("\nPerformance Summary (with clipping):")
print(results)

# Scatter plots (Training, Testing, Combined)
#    All points have non‑negative predictions due to clipping.
dot_size = 30
...
# 5‑fold cross‑validation on training set (with clipping)
#    We clip predictions in each fold as well.
kf = KFold(n_splits=5, shuffle=True, random_state=100)
fold_results = []

# Extract clean parameters from best_params_
best_params_clean = {k.split('__')[1]: v for k, v in grid_search.best_params_.items()}
kernel = best_params_clean.get('kernel', 'rbf')
C = best_params_clean.get('C', 1.0)
epsilon = best_params_clean.get('epsilon', 0.1)
gamma = best_params_clean.get('gamma', 'scale')
degree = best_params_clean.get('degree', 3)

print("\nStart 5-fold cross-validation (with clipping of negative predictions)...\n")
for fold, (train_index, val_index) in enumerate(kf.split(X_train), 1):
    ...
    svr_fold = make_pipeline(
        StandardScaler(),
        SVR(kernel=kernel, C=C, epsilon=epsilon, gamma=gamma, degree=degree)
    )
    svr_fold.fit(X_fold_train, y_fold_train)

    y_fold_train_pred_raw = svr_fold.predict(X_fold_train)
    y_fold_val_pred_raw = svr_fold.predict(X_fold_val)

    # Clip negative predictions to zero
    y_fold_train_pred = np.maximum(y_fold_train_pred_raw, 0)
    y_fold_val_pred = np.maximum(y_fold_val_pred_raw, 0)
    ...
    # Plot validation predictions for this fold (already clipped)
    ...
    plt.title(f'SVR – Cross-validation Fold {fold}', fontsize=13, fontweight='bold')
...
print("\n========== Detailed results of 5-fold cross-validation (with clipping) ==========")
...
def draw_results_table(results_df):
    ...
    table.scale(1.2, 1.5)
    plt.title('5-Fold Cross-Validation Metrics (with Clipping)',
...
# Predictions for 12 materials (t1 to t12) – clipped to non‑negative
...
for t1 in t_values:
    ...
    pred = best_svr.predict(Res)
    predicted_t1.append(np.maximum(pred[0], 0))
...
for t2 in t_values:
    Res = pd.DataFrame([[t2, 248, 0.28, 816, 2.16, 1.484966, 0.046636,
                       ...
                       columns=X_train.columns)
    pred = best_svr.predict(Res)
    predicted_t2.append(np.maximum(pred[0], 0))
...
for t3 in t_values:
    Res = pd.DataFrame([[t3, 187.21, 0.15, 912, 2.29, 1.659668, 0.046636,
                       ...
                       columns=X_train.columns)
    pred = best_svr.predict(Res)
    predicted_t3.append(np.maximum(pred[0], 0))
...
for t4 in t_values:
    Res = pd.DataFrame([[t4, 155, 2.35, 300, 1.7, 0.545943, 0.046636,
                       ...
                       columns=X_train.columns)
    pred = best_svr.predict(Res)
    predicted_t4.append(np.maximum(pred[0], 0))
...
for t5 in t_values:
    Res = pd.DataFrame([[t5, 169, 0.29, 906, 2.21, 1.648749, 0.046636,
                       ...
                       columns=X_train.columns)
    pred = best_svr.predict(Res)
    predicted_t5.append(np.maximum(pred[0], 0))
...
for t6 in t_values:
    Res = pd.DataFrame([[t6, 227.3, 7.2, 1000, 3.22, 1.819811, 0.046636,
                       ...
                       columns=X_train.columns)
    pred = best_svr.predict(Res)
    predicted_t6.append(np.maximum(pred[0], 0))
...
for t7 in t_values:
    Res = pd.DataFrame([[t7, 186.7, 0.2, 897, 2.17, 1.632371, 0.046636,
                       ...
                       columns=X_train.columns)
    pred = best_svr.predict(Res)
    predicted_t7.append(np.maximum(pred[0], 0))
...
for t8 in t_values:
    Res = pd.DataFrame([[t8, 174, 5.112, 1348, 2.06, 2.453106, 0.046636,
                       ...
                       columns=X_train.columns)
    pred = best_svr.predict(Res)
    predicted_t8.append(np.maximum(pred[0], 0))
...
for t9 in t_values:
    Res = pd.DataFrame([[t9, 244.4, 0.6, 1937, 2.9, 3.524975, 0.046636,
                       ...
                       columns=X_train.columns)
    pred = best_svr.predict(Res)
    predicted_t9.append(np.maximum(pred[0], 0))
...
for t10 in t_values:
    Res = pd.DataFrame([[t10, 237.6, 0.52, 1345, 1.27, 2.447646, 0.046636,
                       ...
                       columns=X_train.columns)
    pred = best_svr.predict(Res)
    predicted_t10.append(np.maximum(pred[0], 0))
...
for t11 in t_values:
    Res = pd.DataFrame([[t11, 169, 0.7, 1450, 1.83, 2.638727, 0.046636,
                       ...
                       columns=X_train.columns)
    pred = best_svr.predict(Res)
    predicted_t11.append(np.maximum(pred[0], 0))
...
for t12 in t_values:
    Res = pd.DataFrame([[t12, 333.7, 0.8, 1346, 1.98, 2.449466, 0.046636,
                       ...
                       columns=X_train.columns)
    pred = best_svr.predict(Res)
    predicted_t12.append(np.maximum(pred[0], 0))
...
print("\nPredictions for 12 materials (non‑negative):")
