...
# Define physical monotonic constraints (for documentation only)
...
print("Monotone constraints (order matches features) – NOT APPLIED to Decision Tree:", monotone_constraints)

# Hyperparameter optimization with Optuna (TPE) using scaled data
def objective(trial):
...
# Train final Decision Tree model on scaled data (no constraints)
dt = DecisionTreeRegressor(**study.best_params, random_state=100)
...
# Feature importance (Decision Tree built-in)
...
for fold, (train_index, val_index) in enumerate(kf.split(X_train_scaled), 1):
    ...
    dt_fold = DecisionTreeRegressor(**study.best_params, random_state=100)
...
# SHAP interpretability analysis (for Decision Tree)
