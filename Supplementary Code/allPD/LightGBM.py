...
# LightGBM expects a list or tuple of length equal to number of features
monotone_constraints_list = monotone_constraints

# Hyperparameter optimisation with Optuna (TPE)
# Constraints are NOT applied during search, only in final model and CV
def objective(trial):
...
# Train final LightGBM model WITH monotonic constraints
lgbm = lgb.LGBMRegressor(
    **study.best_params,
    monotone_constraints=monotone_constraints_list,   # apply physical constraints
    random_state=100,
    n_jobs=-1,
    verbosity=-1
)
...
results = pd.DataFrame([['LightGBM (with monotonic constraints)',
...
# 5‑fold cross‑validation on training set (with constraints)
...
for fold, (train_index, val_index) in enumerate(kf.split(X_train_scaled), 1):
    ...
    y_fold_train_pred = lgbm_fold.predict(X_fold_train)
    y_fold_val_pred = lgbm_fold.predict(X_fold_val)
    ...
    plt.title(f'LightGBM – Cross-validation Fold {fold}', fontsize=13, fontweight='bold')
...
for t1 in t_values:
    ...
    predicted = lgbm.predict(New_Res)
