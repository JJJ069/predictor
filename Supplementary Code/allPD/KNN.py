...
# Standardization (KNN requires scaling)
...
print("Monotone constraints (order matches features) – NOT APPLIED to KNN:", monotone_constraints)

# GridSearchCV for KNN (with scaling pipeline)
param_grid = {
    'kneighborsregressor__n_neighbors': [3, 5, 7, 9, 11, 13, 15],
    'kneighborsregressor__weights': ['uniform', 'distance'],
    'kneighborsregressor__p': [1, 2],                     # 1: Manhattan, 2: Euclidean
    'kneighborsregressor__metric': ['minkowski']          # works with p
}
...
grid_search.fit(X_train, y_train)   # X_train is unscaled; pipeline handles scaling
...
results = pd.DataFrame([['KNN (no monotonic constraints)',
...
# 5‑fold cross‑validation on training set (using best parameters, with scaling pipeline)
...
n_neighbors = best_params_clean.get('n_neighbors', 5)
weights = best_params_clean.get('weights', 'uniform')
p = best_params_clean.get('p', 2)
metric = best_params_clean.get('metric', 'minkowski')

print("\nStart 5-fold cross-validation (with best KNN parameters)...\n")
for fold, (train_index, val_index) in enumerate(kf.split(X_train), 1):
    ...
    knn_fold = make_pipeline(
        StandardScaler(),
        KNeighborsRegressor(n_neighbors=n_neighbors, weights=weights, p=p, metric=metric)
    )
    ...
    plt.title(f'KNN – Cross-validation Fold {fold}', fontsize=13, fontweight='bold')
...
for t1 in t_values:
    ...
    predicted = best_knn.predict(Res)   # pipeline scales automatically
...
for t2 in t_values:
    Res = pd.DataFrame([[t2, 248, 0.28, 816, 2.16, 1.484966, 0.046636,
                         0.361195, 0.4, 0.01885, 40, 10, 500]],
                       columns=X_train.columns)
    predicted = best_knn.predict(Res)
