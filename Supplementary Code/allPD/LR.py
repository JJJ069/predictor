...
print("Monotone constraints (order matches features) – NOT APPLIED to Linear Regression:", monotone_constraints)

# GridSearchCV for Linear Regression (with scaling pipeline)
...
param_grid_lr = {
    ...
    'linearregression__copy_X': [True, False]   # optional
}
...
print("\nStarting GridSearchCV for Linear Regression...")
grid_search_lr.fit(X_train, y_train)

print("Best hyperparameters:", grid_search_lr.best_params_)
print("Best cross-validation RMSE:", -grid_search_lr.best_score_)
...
results = pd.DataFrame([['Linear Regression (no monotonic constraints)',
...
textstr = f'Linear Regression: Training (negative preds omitted)\nRMSE: {train_rmse:.4f}\nMAE: {train_mae:.4f}\nR²: {train_r2:.4f}'
...
plt.yticks(fontsize=15, fontname='Arial')
plt.grid(True, linestyle='--', alpha=0.2)
textstr = f'Linear Regression: Testing (negative preds omitted)\nRMSE: {test_rmse:.4f}\nMAE: {test_mae:.4f}\nR²: {test_r2:.4f}'
plt.text(0.05, 0.95, textstr, transform=plt.gca().transAxes, fontsize=15,
         verticalalignment='top', bbox=props, fontname='Arial')
plt.tight_layout()
plt.show()
...
# Extract the scaler and the linear regression model from the pipeline
scaler = best_lr.named_steps['standardscaler']
lr_model = best_lr.named_steps['linearregression']
coefs = lr_model.coef_
# Since features were standardized, coefficients reflect importance.
...
best_params_clean = {k.split('__')[1]: v for k, v in grid_search_lr.best_params_.items()}
fit_intercept = best_params_clean.get('fit_intercept', True)
copy_X = best_params_clean.get('copy_X', True)

print("\nStart 5-fold cross-validation (with best Linear Regression parameters)...\n")
for fold, (train_index, val_index) in enumerate(kf.split(X_train), 1):
    ...
    lr_fold = make_pipeline(
        StandardScaler(),
        LinearRegression(fit_intercept=fit_intercept, copy_X=copy_X)
    )
    lr_fold.fit(X_fold_train, y_fold_train)

    y_fold_train_pred = lr_fold.predict(X_fold_train)
    ...
    plt.title(f'Linear Regression – Cross-validation Fold {fold}', fontsize=13, fontweight='bold')
...
for t1 in t_values:
    ...
    predicted = best_lr.predict(Res)   # pipeline scales automatically
