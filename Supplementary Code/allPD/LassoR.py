...
print("Monotone constraints (order matches features) – NOT APPLIED to Lasso:", monotone_constraints)

# GridSearchCV for Lasso (with scaling pipeline)
pipeline_lasso = make_pipeline(
    StandardScaler(),
    Lasso(random_state=100, max_iter=10000)
)
...
print("Best cross-validation RMSE:", -grid_search_lasso.best_score_)
...
results = pd.DataFrame([['Lasso (no monotonic constraints)',
...
textstr = f'Lasso: Training (negative preds omitted)\nRMSE: {train_rmse:.4f}\nMAE: {train_mae:.4f}\nR²: {train_r2:.4f}'
...
plt.yticks(fontsize=15, fontname='Arial')
plt.grid(True, linestyle='--', alpha=0.2)
textstr = f'Lasso: Testing (negative preds omitted)\nRMSE: {test_rmse:.4f}\nMAE: {test_mae:.4f}\nR²: {test_r2:.4f}'
plt.text(0.05, 0.95, textstr, transform=plt.gca().transAxes, fontsize=15,
         verticalalignment='top', bbox=props, fontname='Arial')
plt.tight_layout()
plt.show()
...
plt.legend(prop=legend_prop, frameon=True, fancybox=True, framealpha=0.6,
           edgecolor='black', facecolor='white', bbox_to_anchor=(0.58, 0.96),
...
# Feature importance: Lasso has coefficients (absolute values as importance)
# Extract the scaler and the lasso model from the pipeline
scaler = best_lasso.named_steps['standardscaler']
...
best_selection = grid_search_lasso.best_params_.get('lasso__selection', 'cyclic')

print("\nStart 5-fold cross-validation (with best Lasso parameters)...\n")
for fold, (train_idx, val_idx) in enumerate(kf.split(X_train), 1):
    ...
    lasso_fold = make_pipeline(
        StandardScaler(),
        Lasso(alpha=best_alpha, selection=best_selection, random_state=100, max_iter=10000)
    )
    lasso_fold.fit(X_fold_train, y_fold_train)

    y_fold_train_pred = lasso_fold.predict(X_fold_train)
    y_fold_val_pred = lasso_fold.predict(X_fold_val)
    ...
    plt.title(f'Lasso – Cross-validation Fold {fold}', fontsize=13, fontweight='bold')
...
for t1 in t_values:
    ...
    predicted = best_lasso.predict(Res)   # pipeline scales automatically
...
for t2 in t_values:
    Res = pd.DataFrame([[t2, 248, 0.28, 816, 2.16, 1.484966, 0.046636,
                         0.361195, 0.4, 0.01885, 40, 10, 500]],
                       columns=X_train.columns)
    predicted = best_lasso.predict(Res)
