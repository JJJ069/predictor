...
print("Monotone constraints (order matches features) – NOT APPLIED to MLP:", monotone_constraints)

# Hyperparameter optimisation with Optuna (TPE) for MLP
def objective(trial):
    hidden_layer_sizes = trial.suggest_categorical('hidden_layer_sizes',
                                                   [(50,), (100,), (50, 50), (100, 50), (100, 100), (50, 100, 50)])
    ...
    neg_rmse = cross_val_score(pipeline, X_train, y_train, cv=cv,
...
# Train final MLP model with best parameters
...
textstr = f'MLP: Training (negative preds omitted)\nRMSE: {train_rmse:.4f}\nMAE: {train_mae:.4f}\nR²: {train_r2:.4f}'
...
plt.yticks(fontsize=15, fontname='Arial')
plt.grid(True, linestyle='--', alpha=0.2)
textstr = f'MLP: Testing (negative preds omitted)\nRMSE: {test_rmse:.4f}\nMAE: {test_mae:.4f}\nR²: {test_r2:.4f}'
plt.text(0.05, 0.95, textstr, transform=plt.gca().transAxes, fontsize=15,
         verticalalignment='top', bbox=props, fontname='Arial')
plt.tight_layout()
plt.show()
...
# 5‑fold cross‑validation (with filtering in validation plots)
kf = KFold(n_splits=5, shuffle=True, random_state=100)
fold_results = []

print("\nStart 5-fold cross-validation (with best MLP parameters)...\n")
for fold, (train_index, val_index) in enumerate(kf.split(X_train), 1):
    ...
    y_fold_train_pred = mlp_fold.predict(X_fold_train)
    y_fold_val_pred = mlp_fold.predict(X_fold_val)
    ...
    if np.any(mask_val):  # only fit line if there are points
        ...
        # fallback diagonal if no points
        plt.plot([0, 1], [0, 1], '--', c='#595959', lw=2)
...
for t1 in t_values:
    ...
    predicted = best_pipeline.predict(Res)
