...
# Monotonic Constraints (not supported by Decision Tree, kept for reference only)
...
print("Monotone constraints (order matches features, NOT used by Decision Tree):", monotone_constraints)

# Hyperparameter Optimization (Optuna) – Decision Tree version
def objective(trial):
    max_depth = trial.suggest_int('max_depth', 3, 20)
    min_samples_split = trial.suggest_int('min_samples_split', 2, 20)
    min_samples_leaf = trial.suggest_int('min_samples_leaf', 1, 20)
    max_features = trial.suggest_float('max_features', 0.3, 1.0)
    ccp_alpha = trial.suggest_float('ccp_alpha', 0.0, 0.05)  # cost complexity pruning
...
# Train Final Decision Tree Model (no monotonic constraints)
...
y_train_pred = predict_nonneg(dt_model, X_train_scaled)
y_test_pred  = predict_nonneg(dt_model, X_test_scaled)
...
results = pd.DataFrame([['Decision Tree (no monotonic constraints)',
...
textstr = f'Decision Tree: Training\nRMSE: {train_rmse:.4f}\nMAE: {train_mae:.4f}\nR²: {train_r2:.4f}'
...
plt.yticks(fontsize=15, fontname='Arial')
plt.grid(True, linestyle='--', alpha=0.2)
textstr = f'Decision Tree: Testing\nRMSE: {test_rmse:.4f}\nMAE: {test_mae:.4f}\nR²: {test_r2:.4f}'
plt.text(0.05, 0.95, textstr, transform=plt.gca().transAxes, fontsize=15,
         verticalalignment='top', bbox=props, fontname='Arial')
plt.tight_layout()
plt.show()
...
# Feature importance (Decision Tree built-in) with value labels
feature_importances = dt_model.feature_importances_
feature_importances_norm = feature_importances / np.sum(feature_importances)
features = X_train.columns
importance_df = pd.DataFrame({
...
for fold, (train_index, val_index) in enumerate(kf.split(X_train_scaled), 1):
    ...
    y_fold_train_pred = predict_nonneg(dt_fold, X_fold_train)
    y_fold_val_pred   = predict_nonneg(dt_fold, X_fold_val)
    ...
    plt.title(f'Decision Tree – Cross-validation Fold {fold}', fontsize=13, fontweight='bold')
...
for t1 in t_values:
    ...
    predicted = predict_nonneg(dt_model, New_Res)
