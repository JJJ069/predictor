...
# Standardization (important for KNN)
...
# Hyperparameter Optimization using GridSearchCV (KNN)
...
# Optionally add 'metric' but we keep default 'minkowski' with p

knn = KNeighborsRegressor()
grid_search = GridSearchCV(
    estimator=knn,
    param_grid=param_grid,
    scoring='neg_root_mean_squared_error',  # same as Optuna objective
    cv=5,
    n_jobs=-1,
    verbose=1
)
...
y_train_pred = predict_nonneg(best_knn, X_train_scaled)
y_test_pred  = predict_nonneg(best_knn, X_test_scaled)
...
results = pd.DataFrame([['KNN (GridSearchCV)',
...
# 5‑Fold Cross‑Validation on best KNN model (using the same best_params)
kf = KFold(n_splits=5, shuffle=True, random_state=100)
fold_results = []

print("\nStart 5-fold cross-validation on best KNN model...\n")
for fold, (train_index, val_index) in enumerate(kf.split(X_train_scaled), 1):
    ...
    knn_fold = KNeighborsRegressor(**grid_search.best_params_)
    knn_fold.fit(X_fold_train, y_fold_train)

    y_fold_train_pred = predict_nonneg(knn_fold, X_fold_train)
    y_fold_val_pred   = predict_nonneg(knn_fold, X_fold_val)
...
for t1 in t_values:
    ...
    predicted = predict_nonneg(best_knn, New_Res)
...
# SHAP interpretability analysis (using KernelExplainer for KNN)
import shap
import matplotlib.pyplot as plt
from matplotlib.colors import LinearSegmentedColormap
import warnings
