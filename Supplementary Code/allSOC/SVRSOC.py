...
# Standardization (important for SVR)
...
# Use a random search over a wide but efficient parameter space
param_dist = {
    'C': loguniform(0.1, 100),
    'gamma': loguniform(0.001, 1),
    'epsilon': uniform(0.01, 0.5)
}
svr = SVR(kernel='rbf')  # rbf is generally the best choice for SVR
random_search = RandomizedSearchCV(
    estimator=svr,
    param_distributions=param_dist,
    n_iter=30,                # Only 30 trials, much faster than full grid
...
y_train_pred = predict_nonneg(best_svr, X_train_scaled)
y_test_pred  = predict_nonneg(best_svr, X_test_scaled)
...
results = pd.DataFrame([['SVR (RandomizedSearchCV)',
...
plt.ylabel('Predicted SOC ×10² (kWh/m³)', fontsize=15, fontname='Arial')
plt.xlabel('Actual SOCy ×10² (kWh/m³)', fontsize=15, fontname='Arial')
plt.xticks(fontsize=15, fontname='Arial')
plt.yticks(fontsize=15, fontname='Arial')
plt.grid(True, linestyle='--', alpha=0.2)
...
print("\nStart 5-fold cross-validation on best SVR model...\n")
for fold, (train_index, val_index) in enumerate(kf.split(X_train_scaled), 1):
    ...
    svr_fold = SVR(**random_search.best_params_)
    svr_fold.fit(X_fold_train, y_fold_train)

    y_fold_train_pred = predict_nonneg(svr_fold, X_fold_train)
    y_fold_val_pred   = predict_nonneg(svr_fold, X_fold_val)
    ...
    # Plot validation predictions for each fold (optional; can be commented out to save time)
...
for t1 in t_values:
    ...
    predicted = predict_nonneg(best_svr, New_Res)
...
# ----------------------------------------------------------------------
# SHAP interpretability analysis (using KernelExplainer for SVR)
# Since SVR is not a tree model, we use KernelExplainer with a background subset.
# ----------------------------------------------------------------------
import shap
import matplotlib.pyplot as plt
from matplotlib.colors import LinearSegmentedColormap
...
# Compute SHAP values for the whole training set (or a subset) – this may take a while.
# We compute for a subset to reduce time, e.g., 100 samples.
