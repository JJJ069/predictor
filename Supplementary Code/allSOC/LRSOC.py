...
# Standardization (optional but recommended for linear models)
...
# Linear Regression does not support monotonic constraints; skip those parts

# Hyperparameter Optimization using GridSearchCV (Linear Regression)
# LinearRegression has very few hyperparameters; we search over 'fit_intercept'
param_grid = {
    'fit_intercept': [True, False]
}
# Note: 'normalize' is deprecated in recent sklearn, we rely on StandardScaler
...
y_train_pred = predict_nonneg(best_lr, X_train_scaled)
y_test_pred  = predict_nonneg(best_lr, X_test_scaled)
...
results = pd.DataFrame([['Linear Regression (GridSearchCV)',
...
textstr = f'Linear Regression: Training\nRMSE: {train_rmse:.4f}\nMAE: {train_mae:.4f}\nR²: {train_r2:.4f}'
...
plt.yticks(fontsize=15, fontname='Arial')
plt.grid(True, linestyle='--', alpha=0.2)
textstr = f'Linear Regression: Testing\nRMSE: {test_rmse:.4f}\nMAE: {test_mae:.4f}\nR²: {test_r2:.4f}'
plt.text(0.05, 0.95, textstr, transform=plt.gca().transAxes, fontsize=15,
         verticalalignment='top', bbox=props, fontname='Arial')
plt.tight_layout()
plt.show()
...
plt.legend(prop=legend_prop, frameon=True, fancybox=True, framealpha=0.6,
           edgecolor='black', facecolor='white', bbox_to_anchor=(0.58, 0.92),
...
# 5‑Fold Cross‑Validation on best Linear Regression model
kf = KFold(n_splits=5, shuffle=True, random_state=100)
fold_results = []

print("\nStart 5-fold cross-validation on best Linear Regression model...\n")
for fold, (train_index, val_index) in enumerate(kf.split(X_train_scaled), 1):
    ...
    lr_fold = LinearRegression(**grid_search.best_params_)
    lr_fold.fit(X_fold_train, y_fold_train)

    y_fold_train_pred = predict_nonneg(lr_fold, X_fold_train)
    y_fold_val_pred   = predict_nonneg(lr_fold, X_fold_val)
...
for t1 in t_values:
    ...
    predicted = predict_nonneg(best_lr, New_Res)
...
# SHAP interpretability analysis (using KernelExplainer for Linear Regression)
# Since Linear Regression is not a tree model, we use KernelExplainer.
import shap
import matplotlib.pyplot as plt
from matplotlib.colors import LinearSegmentedColormap
import warnings
