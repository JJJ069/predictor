...
monotone_constraints_tuple = tuple(monotone_constraints)   # XGBoost requires tuple

# Hyperparameter Optimization (Optuna) – XGBoost version
def objective(trial):
...
y_train_pred = predict_nonneg(xgb_model, X_train_scaled)
y_test_pred  = predict_nonneg(xgb_model, X_test_scaled)
...
# Feature importance (XGBoost built-in) with value labels
feature_importances = xgb_model.feature_importances_
feature_importances_norm = feature_importances / np.sum(feature_importances)
features = X_train.columns
importance_df = pd.DataFrame({
...
for fold, (train_index, val_index) in enumerate(kf.split(X_train_scaled), 1):
    ...
    y_fold_train_pred = predict_nonneg(xgb_fold, X_fold_train)
    y_fold_val_pred   = predict_nonneg(xgb_fold, X_fold_val)
...
for t1 in t_values:
    ...
    predicted = predict_nonneg(xgb_model, New_Res)
