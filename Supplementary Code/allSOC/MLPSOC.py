...
# Monotonic Constraints (not supported by MLP, kept for reference only)
...
print("Monotone constraints (order matches features, NOT used by MLP):", monotone_constraints)

# Hyperparameter Optimization (Optuna) – MLP version
def objective(trial):
    # Hidden layer sizes: up to 3 layers, each with 10-200 neurons
    n_layers = trial.suggest_int('n_layers', 1, 3)
    hidden_layer_sizes = tuple(
        trial.suggest_int(f'layer_{i}_size', 10, 200, step=10) for i in range(n_layers)
    )
...
study.optimize(objective, n_trials=50, show_progress_bar=True)   # reduce trials for speed
print("Best hyperparameters:", study.best_params)

# Train Final MLP Model
# Reconstruct hidden_layer_sizes from the best trial
best_params = study.best_params.copy()
n_layers = best_params.pop('n_layers')
hidden_layer_sizes = tuple(best_params.pop(f'layer_{i}_size') for i in range(n_layers))
...
y_train_pred = predict_nonneg(mlp_model, X_train_scaled)
y_test_pred  = predict_nonneg(mlp_model, X_test_scaled)
...
results = pd.DataFrame([['MLP (no monotonic constraints)',
...
for fold, (train_index, val_index) in enumerate(kf.split(X_train_scaled), 1):
    ...
    # Reconstruct hyperparameters for each fold
    fold_params = study.best_params.copy()
    n_layers_fold = fold_params.pop('n_layers')
    hidden_layer_sizes_fold = tuple(fold_params.pop(f'layer_{i}_size') for i in range(n_layers_fold))
    fold_params['hidden_layer_sizes'] = hidden_layer_sizes_fold
    ...
    y_fold_train_pred = predict_nonneg(mlp_fold, X_fold_train)
    y_fold_val_pred   = predict_nonneg(mlp_fold, X_fold_val)
    ...
    plt.title(f'MLP – Cross-validation Fold {fold}', fontsize=13, fontweight='bold')
...
for t1 in t_values:
    ...
    predicted = predict_nonneg(mlp_model, New_Res)
...
# Use a subset of training data as background for KernelExplainer (to reduce computation)
background = X_train_scaled.sample(n=100, random_state=100)

# Define a prediction function for SHAP (MLPRegressor.predict works directly)
explainer = shap.KernelExplainer(mlp_model.predict, background)
# Compute SHAP values for the entire training set (or a subset for speed)
shap_values = explainer.shap_values(X_train_scaled, nsamples=100)   # nsamples can be reduced
