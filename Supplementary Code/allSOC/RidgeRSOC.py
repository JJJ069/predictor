...
# Hyperparameter Optimization using GridSearchCV (Ridge)
param_grid = {
...
y_train_pred = predict_nonneg(best_ridge, X_train_scaled)
y_test_pred  = predict_nonneg(best_ridge, X_test_scaled)
...
results = pd.DataFrame([['Ridge Regression (GridSearchCV)',
...
plt.xlim(left=0)
plt.ylim(bottom=0)
textstr = f'Ridge Regression: Training\nRMSE: {train_rmse:.4f}\nMAE: {train_mae:.4f}\nR²: {train_r2:.4f}'
...
plt.xlim(left=0)
plt.ylim(bottom=0)
textstr = f'Ridge Regression: Testing\nRMSE: {test_rmse:.4f}\nMAE: {test_mae:.4f}\nR²: {test_r2:.4f}'
plt.text(0.05, 0.95, textstr, transform=plt.gca().transAxes, fontsize=15,
         verticalalignment='top', bbox=props, fontname='Arial')
plt.tight_layout()
plt.show()
...
# 5‑Fold Cross‑Validation on best Ridge model
kf = KFold(n_splits=5, shuffle=True, random_state=100)
fold_results = []

print("\nStart 5-fold cross-validation on best Ridge model...\n")
for fold, (train_index, val_index) in enumerate(kf.split(X_train_scaled), 1):
    ...
    ridge_fold = Ridge(**grid_search.best_params_)
    ridge_fold.fit(X_fold_train, y_fold_train)

    y_fold_train_pred = predict_nonneg(ridge_fold, X_fold_train)
    y_fold_val_pred   = predict_nonneg(ridge_fold, X_fold_val)
    ...
    plt.title(f'Ridge Regression – Cross-validation Fold {fold}', fontsize=13, fontweight='bold')
...
for t1 in t_values:
    ...
    predicted = predict_nonneg(best_ridge, New_Res)
