...
# LightGBM accepts monotone_constraints as a list or tuple
...
def objective(trial):
    ...
    model = LGBMRegressor(
        ...
        n_jobs=-1,
        verbose=-1
        # no monotone_constraints during search
    )
...
y_train_pred = predict_nonneg(lgb_model, X_train_scaled)
y_test_pred  = predict_nonneg(lgb_model, X_test_scaled)
...
# Feature importance (LightGBM built-in) with value labels
...
for fold, (train_index, val_index) in enumerate(kf.split(X_train_scaled), 1):
    ...
    y_fold_train_pred = predict_nonneg(lgb_fold, X_fold_train)
    y_fold_val_pred   = predict_nonneg(lgb_fold, X_fold_val)
...
for t1 in t_values:
    ...
    predicted = predict_nonneg(lgb_model, New_Res)
