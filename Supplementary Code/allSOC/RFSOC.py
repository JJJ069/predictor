...
# Monotonic Constraints (not supported by Random Forest, kept for reference only)
...
print("Monotone constraints (order matches features, NOT used by Random Forest):", monotone_constraints)
...
# Train Final Random Forest Model (no monotonic constraints)
...
y_train_pred = predict_nonneg(rf_model, X_train_scaled)
y_test_pred  = predict_nonneg(rf_model, X_test_scaled)
...
results = pd.DataFrame([['Random Forest (no monotonic constraints)',
...
textstr = f'Random Forest: Training\nRMSE: {train_rmse:.4f}\nMAE: {train_mae:.4f}\nR²: {train_r2:.4f}'
...
plt.yticks(fontsize=15, fontname='Arial')
plt.grid(True, linestyle='--', alpha=0.2)
textstr = f'Random Forest: Testing\nRMSE: {test_rmse:.4f}\nMAE: {test_mae:.4f}\nR²: {test_r2:.4f}'
plt.text(0.05, 0.95, textstr, transform=plt.gca().transAxes, fontsize=15,
         verticalalignment='top', bbox=props, fontname='Arial')
plt.tight_layout()
plt.show()
...
# Feature importance (Random Forest built-in) with value labels
...
for fold, (train_index, val_index) in enumerate(kf.split(X_train_scaled), 1):
    ...
    y_fold_train_pred = predict_nonneg(rf_fold, X_fold_train)
    y_fold_val_pred   = predict_nonneg(rf_fold, X_fold_val)
    ...
    plt.title(f'Random Forest – Cross-validation Fold {fold}', fontsize=13, fontweight='bold')
...
for t1 in t_values:
    ...
    predicted = predict_nonneg(rf_model, New_Res)
